...
@app.post("/v1/chat/completions/{rest}", dependencies=[Depends(user_checks)])
@app.post("/v1/chat/completions", dependencies=[Depends(user_checks)])
async def chat(request: Request, data: ChatBody) -> dict:
    ...
    subscriber = await DatabaseManager.subscriber_check(key)

    if (await moderation(messages)):
        raise HTTPException(detail={"error": {"message": "RP is not allowed here, bitch.", "type": "error", "param": None, "code": 403}}, status_code=403)

    if models in premium_models and not premium:
    ...
    helpers.create_background_task(DatabaseManager.credits_update(key, round((prompt_tokens + completion_tokens) / 5)))
